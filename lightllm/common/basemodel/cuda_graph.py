...
class CudaGraph:
    ...
    def __init__(self, max_batch_size=8, max_len_in_batch=8192, tp_world_size: int = 1):
        ...
        self.normal_cuda_graph_batch_sizes = self._build_cuda_graph_batch_sizes(
        ...
        if self.mtp_step > 0:
            ...
            logger.info(
                f"normal cuda graph batch_sizes: {self.normal_cuda_graph_batch_sizes}"
            )
            logger.info(
                f"mtp verify cuda graph batch_sizes: {self.mtp_verify_cuda_graph_batch_sizes}"
            )
        else:
    ...
    def _build_cuda_graph_batch_sizes(self, batch_size_multiple: int):
        ...
        batch_sizes = [
            i * batch_size_multiple
            for i in range(1, self.args.graph_split_batch_size + 1)
        ]
        ...
        if self.args.enable_tpsp_mix_mode:
            ...
            batch_sizes = [
                triton.cdiv(e, padding_unit) * padding_unit for e in batch_sizes
            ]
    ...
    def _decode_graph_key(self, infer_state: InferStateInfo):
        is_mtp_verify_decode = (
            self.mtp_step > 0 and infer_state.b_num_accepted_tokens is not None
        )
        return (infer_state.input_ids.shape[0], is_mtp_verify_decode)

    def need_capture(self, batch_size, is_mtp_verify_decode=False):
        find_batch_size = self.find_closest_graph_batch_size(
            batch_size, is_mtp_verify_decode=is_mtp_verify_decode
        )
    ...
    def find_closest_graph_batch_size(self, batch_size, is_mtp_verify_decode=False):
        graph_batch_sizes = self._get_graph_batch_sizes(
    ...
    def _build_warmup_decode_model_input(
        ...
        if self.mtp_step > 0 and is_mtp_verify_decode:
            assert (
                batch_size % mtp_size == 0
            ), "MTP decode CUDA graph batch size must be a multiple of mtp_step + 1"
            real_batch_size = batch_size // mtp_size
            b_mtp_index = torch.arange(
                mtp_size, dtype=torch.int32, device=device
            ).repeat(real_batch_size)
            b_seq_len = torch.arange(
                2, mtp_size + 2, dtype=torch.int32, device=device
            ).repeat(real_batch_size)
            b_num_accepted_tokens = torch.ones(
                real_batch_size, dtype=torch.int32, device=device
            )
    ...
    def _replay(self, infer_state: InferStateInfo):
        graph_obj, graph_infer_state, graph_output = self.graph[
            self._decode_graph_key(infer_state)
        ]
    ...
    def warmup_overlap(self, model):
        ...
        for is_mtp_verify_decode, batch_sizes in self._iter_warmup_graph_layouts(model):
            for batch_size in batch_sizes[::-1]:
                ...
                _, _ = model.microbatch_overlap_decode(
                    decode_batches[0], decode_batches[1]
                )
