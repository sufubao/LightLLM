...
@torch.no_grad()
def repack_kv_index(kv_index, req_index, seq_len, start_loc, max_seq_len, out_kv_index, zero_output: bool = True):
    batch_size = req_index.shape[0]
    # Some flashinfer callers need zero-filled padding outside the valid indptr range.
    if zero_output:
        out_kv_index.zero_()
    BLOCK = 64
    grid = (
