...
def fused_recurrent_kda_packed_decode_kernel(
    ...
    o_d = tl.arange(0, D)
    o_v = i_v * BD + tl.arange(0, BD)
    m_v = o_v < D
    m_state = m_v[:, None]

    state_idx = tl.load(state_indices + i_n * stride_indices).to(tl.int64)
    p_o = o + (i_n * H + i_h) * D + o_v
    if state_idx < 0:
        tl.store(p_o, 0.0, mask=m_v)
        return

    p_qkv = mixed_qkv + i_n * stride_qkv_token
    head_offset = i_h * D
    b_q = tl.load(p_qkv + head_offset + o_d).to(tl.float32)
    b_k = tl.load(p_qkv + H * D + head_offset + o_d).to(tl.float32)
    b_v = tl.load(p_qkv + 2 * H * D + head_offset + o_v, mask=m_v, other=0.0).to(tl.float32)
    ...
    p_state = state + state_idx * stride_state_token + i_h * D * D + o_v[:, None] * D + o_d[None, :]
    b_state = tl.load(p_state, mask=m_state, other=0.0).to(tl.float32)
    b_gate = tl.load(g + (i_n * H + i_h) * D + o_d).to(tl.float32)
    b_state *= exp(b_gate[None, :])
    b_v -= tl.sum(b_state * b_k[None, :], axis=1)
    b_v *= tl.load(beta + i_n * H + i_h).to(tl.float32)
...
def fused_recurrent_kda_packed_decode(
    ...
    num_heads = initial_state.shape[1]
    assert mixed_qkv.shape == (batch, 3 * num_heads * head_dim)
    assert g.numel() == batch * num_heads * head_dim
    assert beta.numel() == batch * num_heads
    assert initial_state.shape[2:] == (head_dim, head_dim)
    assert ssm_state_indices.shape == (batch,)
    if scale is None:
        scale = head_dim**-0.5

    output = mixed_qkv.new_empty((batch, 1, num_heads, head_dim))
    block_value = min(next_power_of_2(head_dim), 8)
    grid = (cdiv(head_dim, block_value), batch * num_heads)
    fused_recurrent_kda_packed_decode_kernel[grid](
        ...
        scale=scale,
        stride_qkv_token=mixed_qkv.stride(0),
        stride_state_token=initial_state.stride(0),
        stride_indices=ssm_state_indices.stride(0),
        H=num_heads,
        D=head_dim,
        BD=block_value,
        USE_QK_L2NORM_IN_KERNEL=use_qk_l2norm_in_kernel,
