...
class InferenceContext:
    ...
    def copy_linear_att_state_to_cache_buffer(self, b_req_idx: torch.Tensor, reqs: List["InferReq"]):
        ...
        if any(buffer_id != -1 for buffer_id in big_page_buffer_ids):
            ...
            # Per-req post-accept count, aligned with b_req_idx / reqs.
            b_num_accepted_tokens = torch.tensor(
                [req.mtp_accept_len for req in reqs],
                dtype=torch.int32,
                requires_grad=False,
                device="cpu",
        ...
        for req in reqs:
            ...
            if req.get_chuncked_input_token_len() == req.linear_att_cache_len:
                ...
                if req.linear_att_cache_len % big_page_token_num != 0:
                    ...
                    if req.tail_linear_att_small_page_buffer_id is not None:
                        # Asymmetric layout (design §3.4): conv is one widened slot
                        # keyed by req_idx; read the narrow committed window at
                        # canonical offset (accept_len-1). SSM is an (S+1) block;
                        # read the canonical block slot.
...
class InferReq:
    ...
    def __init__(
        ...
        # Carried per-request canonical pointer for hybrid-MTP conv/SSM state
        # (design §3.1). This is the PREVIOUS step's accept count, passed to the
        # GDN verify + conv kernels as num_accepted_tokens. Distinct from the
        # stats accumulator update_mtp_accepted_token_num (which only sums for
        # accept-ratio reporting). Reset to 1 on new req and after cache restore;
        # kernels do (num_accepted - 1) so it must always be >= 1.
