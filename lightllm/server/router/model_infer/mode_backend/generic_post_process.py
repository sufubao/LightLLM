...
def can_use_vocab_parallel_greedy(reqs: List[InferReq]) -> bool:
    return is_vocab_parallel_greedy_enabled() and _can_use_unmodified_greedy_logits(reqs)


def sample(model_output: ModelOutput, reqs: List[InferReq], eos_id: List[int] = [2]):
    logits = model_output.logits
    if model_output.has_vocab_parallel_logits:
        if not _can_use_unmodified_greedy_logits(reqs):
            raise RuntimeError("vocab-parallel logits require unmodified greedy requests")
        candidate_indexes = torch.argmax(logits, dim=-1, keepdim=True)
        token_ids = model_output.logits_token_ids.gather(1, candidate_indexes).view(-1).long()
        selected_logits = logits.gather(1, candidate_indexes).view(-1)
        return token_ids, selected_logits - model_output.logits_logsumexp
