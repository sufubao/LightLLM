...
def sample(
    ...
    if is_all_greedy:
        ...
        batch_next_token_logprobs = torch.log(batch_next_token_probs).view(-1)
    ...
    if logits_token_ids is not None:
        batch_next_token_ids = logits_token_ids.gather(1, batch_next_token_ids.long().view(-1, 1))
...
def _get_post_sample_tensors(reqs: List[InferReq], logits_width: Optional[int] = None):
    req_idxes: List[int] = []
    ...
    for req_obj in reqs:
        ...
        top_k_val = shm_param.top_k if logits_width is None else min(shm_param.top_k, logits_width)
        top_ks.append(top_k_val)
        if top_k_val > 1:
            is_all_greedy = False
        if top_k_val != (req_obj.vocab_size if logits_width is None else logits_width):
            skip_top_k = False
        if shm_param.top_p != 1.0:
            skip_top_p = False
        if req_obj.generator is not None:
            exist_req_use_random_seed = True
        if logits_width is None:
            ...
            invalid_token_num_start += len(sample_param.invalid_token_ids)
    ...
    req_idxes_gpu = length_penalty_param_gpu = mask_eos_reqs_gpu = None
    if logits_width is None:
        req_idxes_gpu = g_pin_mem_manager.gen_from_list(key="req_idxes", data=req_idxes, dtype=torch.int32).cuda(
            non_blocking=True
        )
        length_penalty_param_gpu = g_pin_mem_manager.gen_from_list(
            key="length_penalty_param", data=length_penalty_param, dtype=torch.int32
        ).cuda(non_blocking=True)
        mask_eos_reqs_gpu = g_pin_mem_manager.gen_from_list(
            key="mask_eos_reqs", data=mask_eos_reqs, dtype=torch.bool
        ).cuda(non_blocking=True)
