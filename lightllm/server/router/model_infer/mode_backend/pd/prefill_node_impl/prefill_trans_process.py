    ...
    def _update_task_status_once(self):
        with self.waiting_dict_lock:
            ...
            for trans_task in tasks:
                ...
                # A single native NIXL error must fail and release this task without
                # silently terminating the status thread for every later transfer.
                try:
                ...
                if ret == "DONE":
                    ...
                    if self.transporter.capture_telemetry:
                        try:
                            telem = self.transporter.get_xfer_telemetry(completed_task.xfer_handle)
                            total_us = telem.xferDuration
                            post_us = telem.postDuration
                            backend_us = telem.xferDuration - telem.postDuration
                            nixl_backend = self.transporter.query_xfer_backend(completed_task.xfer_handle)
                            logger.info(
