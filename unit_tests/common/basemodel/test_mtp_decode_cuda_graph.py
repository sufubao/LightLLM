...
def test_mtp_decode_cuda_graph_warmup_supports_normal_layout_for_draft():
    ...
    model = SimpleNamespace(
        req_manager=SimpleNamespace(HOLD_REQUEST_ID=99),
        mem_manager=FakeMemManager(),
        _gen_special_model_input=lambda token_num: {
            "mtp_draft_input_hiddens": torch.full((token_num, 4), 3.0)
        },
    )
...
def test_mtp_decode_cuda_graph_warmup_layouts_split_main_and_draft_models():
    ...
    assert list(graph._iter_warmup_graph_layouts(Qwen3_5MOETpPartModel())) == [
        (True, [3, 6, 9])
    ]
    assert list(graph._iter_warmup_graph_layouts(Qwen3_5MoeMTPModel())) == [
        (False, [1, 2, 4, 8])
    ]
...
def test_mtp_decode_warmup_layout_marks_qwen3next_verify(monkeypatch):
    ...
    monkeypatch.setattr(
        infer_struct_mod, "get_env_start_args", lambda: SimpleNamespace(mtp_step=2)
    )

    state = Qwen3NextInferStateInfo()
    state.is_prefill = False
    state.b_req_idx = torch.tensor([5, 5, 5, 6, 6, 6], dtype=torch.int32, device="cuda")
    state.b_mtp_index = torch.tensor(
        [0, 1, 2, 0, 1, 2], dtype=torch.int32, device="cuda"
    )
...
def test_mtp_decode_padding_preserves_verify_groups(monkeypatch):
    ...
    monkeypatch.setattr(
        basemodel_mod, "enable_diverse_mode_gqa_decode_fast_kernel", lambda: False
    )
...
def test_qwen3next_hybrid_mtp_keeps_decode_cuda_graph_enabled(monkeypatch):
    ...
    monkeypatch.setattr(
        qwen3next_model, "get_env_start_args", lambda: SimpleNamespace(mtp_step=2)
    )
...
def test_fa3_decode_uses_normal_layout_for_narrowed_mtp_draft(monkeypatch):
    ...
    monkeypatch.setattr(
        fa3_fp, "get_env_start_args", lambda: SimpleNamespace(mtp_step=2)
    )
    ...
    model = SimpleNamespace(
        ...
        req_manager=SimpleNamespace(
            req_to_token_indexs=torch.empty((8, 32), dtype=torch.int32)
        ),
    )
