...
def test_tuner_declares_mutated_state():
    assert mtp_recurrent.mtp_fused_recurrent_gated_delta_rule.mutates_args == ["initial_state"]


def test_gdn_cuda_graph_warmup_enables_v1_autotune_once(monkeypatch):
    ...
    infer_state = SimpleNamespace(is_cuda_graph=True)
    ...
    assert backend._mtp_autotuned_shapes == {(8, 2, 4)}
...
def _launch(case, run_config, *, fixed_seq_len):
    ...
    output, _ = mtp_recurrent.mtp_fused_recurrent_gated_delta_rule(**kwargs)
...
def test_tuner_warmup_does_not_mutate_caller_state():
    ...
    tuner = mtp_recurrent.mtp_fused_recurrent_gated_delta_rule
